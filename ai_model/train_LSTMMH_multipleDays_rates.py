...
def load_and_preprocess_data(args):
    """
    변경 요약
    - 수익률(return) 계산 후 dropna().
    - 시퀀스(X, y, dates, base_prices)는 '비스케일'로 먼저 생성.
    - 날짜 기준으로 train/val/test 분할을 한 뒤,
      오직 train으로만 스케일러를 fit, 각 분할에 transform 적용.
    - target은 (return) 단일 피처이므로 scaler.transform 시 reshape(-1,1) 사용.
    """
    ...
    # 2) 수익률 계산(타겟), dropna 후 인덱스 재정렬
    original_target_prices = df[args.target_column].copy()
    df['return'] = df[args.target_column].pct_change()
    df = df.dropna().reset_index(drop=True)
    original_target_prices = original_target_prices.iloc[1:].reset_index(drop=True)

    # 3) 피처/타겟 원본 배열(비스케일)
    features_df = df[args.feature_columns]
    target_series = df['return']
    raw_features = features_df.values.astype(np.float32)                  # (N, F)
    raw_target = target_series.values.astype(np.float32).reshape(-1, 1)   # (N, 1)
    ...
    for i in range(N - T - H + 1):
        X.append(raw_features[i: i + T])                                  # (T, F)
        y.append(raw_target[i + T: i + T + H].squeeze())                  # (H,)
        dates.append(df['Date'].iloc[i + T])
        base_prices.append(original_target_prices.iloc[i + T - 1])

    X = np.array(X)                                # (M, T, F)
    y = np.array(y)                                # (M, H)
    dates = pd.Series(dates).reset_index(drop=True)
    base_prices = np.array(base_prices)            # (M,)

    # 5) 날짜 기준 split (train/val/test)
    ...
    print("--------------------------\n")
    print("y_train mean per horizon (scaled):", np.mean(y_train, axis=0).round(6))
    print("y_train std  per horizon (scaled):", np.std(y_train,  axis=0).round(6))
    print()
...
def evaluate_model(model, test_loader, target_scaler, device, test_base_prices):
    ...
    with torch.no_grad():
        for X_batch, y_batch in test_loader:
            ...
            outputs_scaled_return = model(X_batch).cpu().numpy()           # (B, H)
            actuals_scaled_return = y_batch.numpy()                        # (B, H)

            # 2) 스케일러는 1피처로 fit 되었으므로 (-1,1)로 펴서 역변환 후 복원
            B, H = outputs_scaled_return.shape
            outputs_return = target_scaler.inverse_transform(
            outputs_scaled_return.reshape(-1, 1)
            ).reshape(B, H)
            actuals_return = target_scaler.inverse_transform(
            actuals_scaled_return.reshape(-1, 1)
            ).reshape(B, H)
