...
def train(model, train_loader, val_loader, device, epochs=50, lr=1e-3):
    ...
    for ep in range(1, epochs+1):
        ...
        print(f"Epoch {ep:03d} | Train Huber: {tr:.6f} | Val Huber: {va:.6f}")
...
if __name__ == "__main__":
    ...
    model  = LSTMRegressor(input_size=1, hidden_size=HIDDEN, num_layers=LAYERS,
                           dropout=DROPOUT, output_size=1)
    train(model, train_loader, test_loader, device, epochs=EPOCHS, lr=LR)

    # 학습 끝난 직후에 추가
    config = {
        "framework": "pytorch",
        "target": "log_returns",           # C 방식
        "input_size": 1,
        "output_size": 1,
        "hidden_size": HIDDEN,
        "num_layers": LAYERS,
    ...
    run_dir = f"runs/usdkrw_lstm_C_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    example_input = torch.randn(1, SEQ_LEN, 1).to(device)

    save_artifacts(
        run_dir=run_dir,
        model=model,
        scaler=scaler,              # 수익률 스케일러(returns용)
        config=config,
        example_input=example_input,
        save_torchscript=False      # True로 바꾸면 model_ts.pt도 저장
    )
    print(f"✅ 모델 저장 완료: {run_dir}")


    # 7) 테스트 예측(수익률) → 역스케일링 → **가격 복원**
    pred_ret_s = predict(model, X_test, device)             # (N_test,1) scaled r̂
    true_ret_s = y_test                                     # (N_test,1) scaled r
    pred_ret   = scaler.inverse_transform(pred_ret_s)       # r̂
    true_ret   = scaler.inverse_transform(true_ret_s)       # r

    # 수익률의 날짜축에서 테스트 시작 위치 (returns 기준)
    test_start_ridx = SEQ_LEN + split

    # 복원: P̂_t = P_{t-1} * exp(r̂_t)
    prev_prices  = prices[test_start_ridx : test_start_ridx + len(pred_ret)]          # (N_test,1)
    pred_prices  = prev_prices * np.exp(pred_ret)                                     # (N_test,1)
    true_prices  = prices[test_start_ridx + 1 : test_start_ridx + 1 + len(pred_ret)]  # (N_test,1)

    # 8) 지표 (가격 & 수익률)
    print("\n[Price Metrics] (Actual vs Reconstructed Pred)")
    mae  = mean_absolute_error(true_prices[:, 0], pred_prices[:, 0])
    mse  = mean_squared_error(true_prices[:, 0], pred_prices[:, 0])  # 이름만 MSE로 맞춤
    r2   = r2_score(true_prices[:, 0], pred_prices[:, 0])
    print(f"{TARGET_COL} | MAE: {mae:.6f} | MSE: {mse:.6f} | R²: {r2:.4f}")

    print("\n[Return Metrics] (y vs ŷ)")
    r_mae  = mean_absolute_error(true_ret[:, 0], pred_ret[:, 0])
    r_mse  = mean_squared_error(true_ret[:, 0], pred_ret[:, 0])
    r_r2   = r2_score(true_ret[:, 0], pred_ret[:, 0])
    print(f"Return | MAE: {r_mae:.8f} | MSE: {r_mse:.8f} | R²: {r_r2:.4f}")

    # 9) 플롯 (가격 복원 결과 단일 축)
    test_dates = r_dates[test_start_ridx : test_start_ridx + len(pred_prices)]
    plt.figure(figsize=(11, 4))
    plt.plot(test_dates, true_prices[:, 0], label="Actual")
    plt.plot(test_dates, pred_prices[:, 0], label="Predicted")
    plt.ylabel(TARGET_COL)
    plt.xlabel("Date")
    plt.title(f"LSTM (Target = Log Returns, Price Reconstructed) — {TARGET_COL}")
    plt.legend(loc="best")
    plt.tight_layout()
    plt.show()
